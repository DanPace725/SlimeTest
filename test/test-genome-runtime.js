...
for (const testCase of fixtures) {
  try {
    const hashes = runGenomeFixtureCase(testCase, { enabled: true });
    ...
    console.log(`Genome determinism case '${testCase.label}' passed (${hashes.length} steps).`);
  } catch (err) {
    failures += 1;
    console.error(`Genome determinism case '${testCase.label}' failed:`, err.message);
  }
}

if (failures > 0) {
  console.error(`Genome determinism tests failed (${failures} cases).`);
  process.exit(1);
}

console.log('Genome determinism tests passed.');
